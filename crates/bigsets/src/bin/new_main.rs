use bigsets::{
    ApiServer, Config, CoreServer, ReplicationManager, ReplicationServer, ServerWrapper,
    SqliteStorage,
};
use std::sync::Arc;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let storage = Arc::new(RwLock::new(SqliteStorage::open(
        &config.server.db_path,
        &config.storage,
    )?));

    // 2. Create core server (business logic)
    let server = Arc::new(CoreServer::new(config.server.actor_id(), Arc::clone(&storage)).await?);
